...
import { type NextFireInfo, nextCalendarOccurrence } from '$lib/workflow/preview-core.js';
import {
	evaluateDateTrigger,
...
export async function computeNextFireInfo(
	rule: {
		id: string;
				...
				for (const tracker of trackers) {
					...
					if (normalizedTrigger.kind === 'maintenance') {
						...
						if (evaluation.matches) {
							hasReady = true;
						} else if (!bestDateFireAt || evaluation.fireAt < bestDateFireAt) {
